...
def pre_process():
    ''' 
        provide the related parameters for BM25 model
        1. Find all documents, and 2&3. use stopword removal, and stemming
        4. calculate related parameters
    '''

    # step 1: extracting the documents
    dirlist = os.listdir('documents') # get all names to iterate;
    N = 0 # N is the total number of documents in the collection. 
    ...
    for dirname in dirlist:
        ...
        if not os.path.isdir(path): # in case this is not a folder, e.g. .DS_Store
            continue
        filelist = os.listdir(path)
        N += len(filelist) # count the total number
...
def load_qrels():
    ...
    # its value is a dict.
    # if relevant that uses the DOCUMENT ID as key, 
    ...
    with open( 'files/qrels.txt', 'r' ) as f:
            ...
            for row in f.read().split('\n'):
                ...
                if int( row[3] ) == 0:
                    ...
                    if row[0] not in non_relevant:
                        non_relevant[row[0]] = set()
                    non_relevant[row[0]].add(row[2])
                else:
                    # first time I've seen the QUERY ID
...
# formula: rel&ret / ret [:15] first 15
...
def get_bpref(retrieved, relevant, non_relevant):
    n = len(relevant)
    ...
    for did in retrieved:
        ...
        if did in relevant:
            value += ( 1 - ( non_relevant_number / n))
        elif did in non_relevant: # when non-relevant, we need add non-rel value
            non_relevant_number += 1
            # when the non-rel number is larger and equal to the relevant number
            if non_relevant_number >= n:
                break
    return value/n
...
def get_ndcg_at_n(retrieved, relevant, n):
    ...
    for index in range(n):
        ...
        if index < len(sorted_relevant_list):
            # IG: relevant[ sorted_relevant_list[index] ]
...
if __name__ == '__main__':
    ...
    if args.mode == 'manual':
        ...
        # get judged relevant and non-relevant documents for each query
        relevant, non_relevant = load_qrels()
        ...
        with open( 'files/output.txt' , 'w' ) as f:
            ...
            for query_id in queries:
                # get the retrieved did set & sort it in a descending order
                ...
                bpref += get_bpref(rank_list, relevant[query_id], non_relevant[query_id])
